...
static void print_sequence_list(void) {
    ...
    for (int i = 0; i < sequence_list.size; i++)
        if (sequence_list.chunk_sequence_len_list[i] > 0)
            printf("idx %d times %d cnt %d\n", i, sequence_list.chunk_sequence_len_list[i], i == sequence_list.size - 1 ? sequence_list.long_chunk_cnt : sequence_list.chunk_sequence_len_list[i] * i);
...
static void init_chunk_boundary_list() {
    chunk_boundary_list = (struct chunk_boundary *)calloc(num_threads,
		 sizeof(struct chunk_boundary) * MAX_CHUNK_IN_FILE);
...
static inline struct fp_entry_st *new_fp_entry(uint8_t *hash, int len) {
    struct fp_entry_st *p = (struct fp_entry_st *)calloc(1, 
			sizeof(struct fp_entry_st) + list_length * sizeof(int));  // next_sizes
    assert(p);
...
static void print_stats(char *opt) {
    ...
    if (strlen(opt) > 0) {
        printf("Output: \n \
			mini_chunksize %u;\n \
			max_chunksize %u;\n \
			expect_avg_chunksize %u;\n \
			magic_number %d %s;\n \
			bench %s;\n",  
			min_chunksize, max_chunksize,
			1 << break_mask_bit, magic_number,
			opt, bench_name);
    } else {
        printf("Output:;\n \
			mini_chunksize %u;\n \
			...
			bench %s;\n",
			min_chunksize, max_chunksize,
			1 << break_mask_bit, magic_number, bench_name);
    }
    if (jump_with_mark_test)
        mode = JM_MODE;
    else if (blind_jump)
    ...
    printf("\
			...
			fast_miss_chunks %lu; \n \
			fast_false_hit_chunks %lu; \n \
			unique_FP %lu; \n \
			num_rollings %lu; \n \
			num_fingerprintings %lu; \n \
			num_io %lu; \n \
			num_path_switch %lu; \n",
			...
			dedup_stats.num_fingerprintings, dedup_stats.num_io,
			dedup_stats.num_path_switch);
    ...
    printf("\
			...
			dedup_stats.fast_cdtime / 1000,
			dedup_stats.iotime / 1000,
			num_threads);
    // print_chunk_dist();
    print_lru_hits();
}
